"""New World NZ promotional campaign detector.

Foodstuffs-family site (same parent as PAK'nSAVE).
Behind Cloudflare — requires Playwright.
"""
...
from config.settings import NEWWORLD_SPECIALS_URL
...
_IGNORE = frozenset({
    "specials", "special", "browse", "shop", "search",
    "categories", "menu", "home", "sign in", "register",
})


class NewWorldScraper(BaseScraper):
    source_name = "newworld"

    def scrape(self) -> list:
        return []

    def run_promos(self) -> List[SupermarketPromo]:
        """Detect current New World promotional campaigns."""
        try:
            return self._scrape_page()
        except Exception as e:
            logger.error(f"New World promo scrape failed: {e}")
    ...
    def _scrape_page(self) -> List[SupermarketPromo]:
        try:
            ...
            logger.warning("Playwright not installed — cannot scrape New World")
            return self._fallback_entry()
                ...
                for t in texts:
                    ...
                    if any(kw in tl for kw in campaign_kw):
                        promos.append(SupermarketPromo(
                            ...
                            date_start=mon,
                            date_end=sun,
                            url=NEWWORLD_SPECIALS_URL,
                        ))

                browser.close()
        except Exception as e:
            logger.error(f"New World Playwright failed: {e}")
        ...
        logger.info(f"New World: {len(promos)} campaigns")
        return promos
